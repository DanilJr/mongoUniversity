...
def get_db():
    ...
    if db is None:
        ...
        db = g._database = MongoClient(MFLIX_DB_URI, maxPoolSize=50, connectTimeoutMS=2500)['sample_mflix']
...
def get_movies_by_country(countries):
    ...
    try:
        ...
        for country in countries:
            result = result + list(db.movies.find({'countries': country}, {'title': 1}))
        return result
...
def get_movies(filters, page, movies_per_page):
    ...
    #movies = cursor.skip(num_to_skip)
    # if page == 0:
    #
    else:
        total_num_movies = db.movies.count_documents(query)
        num_to_skip = page * movies_per_page
        movies = cursor.skip(num_to_skip).limit(movies_per_page)
...
def add_user(name, email, hashedpw):
    ...
    try:
        ...
        inserted = db.users.with_options(write_concern=WriteConcern(w='majority')).insert_one({
            "name": name,
            "email": email,
            "password": hashedpw})
        return {"success": True}
...
def login_user(email, jwt):
    ...
    try:
        ...
        db.sessions.update_one(
            {"user_id": email, 'jwt': jwt},
            {"$set": {"jwt": jwt, "user_id": email}}, upsert=True)
        return {"success": True}
    except Exception as e:
        return {"error": e}
...
def update_prefs(email, prefs):
    ...
    try:
        ...
        response = db.users.update_one(
            {"email": email},
            {"$set": {'preferences': prefs}}, upsert=True)
